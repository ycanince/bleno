...
Bleno.prototype.startAdvertisingIBeacon = function(name, uuid, major, minor, measuredPower, callback) {
  if (this.state !== 'poweredOn') {
    var error = new Error('Could not start advertising, state is ' + this.state + ' (not poweredOn)');
    ...
    this._bindings.startAdvertisingIBeacon(name, iBeaconData);
  }
};
...
Bleno.prototype.disconnect = function(handle) {
  debug('disconnect');
